 ...
 *
 * 3.5.G split: `AiCriticDialog` is the reusable modal (runs the review on
 * mount when no cached report is passed in); `AiCriticButton` stays the
 * header trigger, and the left-sidebar AI hub mounts the dialog from its own
 * card. Each trigger keeps its own report cache.
 */
...
export function AiCriticDialog({
  report,
  onReport,
  onClose,
}: {
  report: CriticReport | null;
  onReport: (report: CriticReport) => void;
  onClose: () => void;
}) {
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="rounded-lg border border-border px-3 py-2 text-sm text-foreground transition hover:bg-accent"
              >
                {t("Try again", "ลองใหม่")}
              </button>
            </div>
